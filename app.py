...
Version stable sans imblearn (RandomOverSampler maison)
"""
...
# ── Remplacement de imblearn : sur‑échantillonnage maison ─────────
def random_oversample(X, y, random_state=42):
    """Sur-échantillonnage aléatoire (classe minoritaire dupliquée)"""
    ...
    return X_res[shuffle].astype(np.float64), y_res[shuffle].astype(np.int64)
...
# ══════════════════════════════════════════════════════════════════
# CSS (inchangé, gardez votre style)
# ══════════════════════════════════════════════════════════════════
st.markdown("""
<style>
...
# ══════════════════════════════════════════════════════════════════
# PIPELINE COMPLET (SANS IMBLEARN)
# ══════════════════════════════════════════════════════════════════
def pipeline_complet(df_raw: pd.DataFrame):
    ...
    # ✅ Sur‑échantillonnage maison (remplace RandomOverSampler)
    ...
    # Vérification rapide des NaN / inf
    if np.any(np.isnan(X_tr)) or np.any(np.isinf(X_tr)):
        st.error("NaN/inf détecté dans X après oversampling")
        st.stop()
...
def plot_learning_curve(estimator, X, y, nom):
    ...
    ax.fill_between(sizes, tr_mean - np.std(tr_scores, axis=1),
                    tr_mean + np.std(tr_scores, axis=1), alpha=0.15, color="#667eea")
    ax.fill_between(sizes, val_mean - np.std(val_scores, axis=1),
                    val_mean + np.std(val_scores, axis=1), alpha=0.15, color="#f5576c")
...
if not st.session_state.model_entraine and st.session_state.df_train is not None:
    ...
    st.session_state.update(dict(
        ...
        y_pred=y_pred, accuracy=accuracy, col_names=col_names,
        model_entraine=True,
    ))
    st.rerun()
...
with st.sidebar:
    ...
    if st.session_state.model_entraine:
        ...
        for i, (nom, res) in enumerate(sorted(ar.items(), key=lambda x: x[1]["auc"], reverse=True), 1):
...
# ══════════════════════════════════════════════════════════════════
# PAGES 1,2,3,4 (inchangées, trop longues à recopier mais elles sont identiques à votre code original)
# Assurez-vous simplement que le nom des variables est cohérent (st.session_state.reponses, etc.)
# ══════════════════════════════════════════════════════════════════

# (J’ai dû raccourcir ici pour la lisibilité, mais vous devez conserver la partie interface utilisateur
#  que vous aviez écrite – les 4 pages avec les questions, les résultats et les graphiques.
#  Cependant, je vous fournis ci‑dessous le code complet des pages 1 à 4 pour être certain.)
    ...
    with bc2:
        if st.button("📝 Commencer le questionnaire", type="primary", use_container_width=True):
            if nom and nom.strip():
                st.session_state.infos_enfant = dict(nom=nom, age=age, genre=genre, ethnie=ethnie, jaundice=jaundice, family_asd=family_asd)
...
# ========= PAGE 3 ==================================================
    ...
    QUESTIONS = [
        ("A1", "😊", "Difficultés à comprendre les expressions faciales ?", "Ne comprend pas quand quelqu'un est triste, content ou fâché"),
        ("A2", "💬", "Difficultés à maintenir une conversation ?", "Ne sait pas quand parler, quand s'arrêter, change de sujet brusquement"),
        ("A3", "🔄", "Comportements répétitifs ?", "Se balance, tourne, tape des mains, répète les mêmes mots"),
        ("A4", "🎯", "Intérêts très spécifiques et intenses ?", "Toujours le même sujet, collectionne des objets inhabituels"),
        ("A5", "😐", "Semble distant ou sans émotion ?", "Ne réagit pas quand on l'appelle, semble dans sa bulle"),
        ("A6", "🔊", "Sensibilité aux bruits ou textures ?", "N'aime pas l'aspirateur, les étiquettes, certaines lumières"),
        ("A7", "🎮", "Préfère jouer seul ?", "Ne cherche pas à faire des amis, joue en solitaire"),
        ("A8", "📖", "Langage très littéral ?", "Ne comprend pas les blagues, l'ironie ou les métaphores"),
        ("A9", "👀", "Évite le contact visuel ?", "Ne regarde pas dans les yeux, détourne le regard"),
        ("A10","📅", "Très attaché à ses routines ?", "Se fâche quand on change ses habitudes ou son environnement"),
    ]
    ...
    for idx, (qid, icon, question, detail) in enumerate(QUESTIONS, 1):
        ...
        with qc2:
            ...
            st.caption(f"💡 {detail}")
            rep = st.radio("", [0,1], format_func=lambda x:"❌ Non" if x==0 else "✅ Oui", key=f"q_{qid}", index=st.session_state.reponses.get(qid, None), horizontal=True, label_visibility="collapsed")
            if rep is not None:
    ...
    if total_rep > 0:
        ...
        <div style="background:linear-gradient(135deg,#667eea,#764ba2); border-radius:20px;padding:1rem;text-align:center;margin:1rem 0;">
            <span style="color:white;font-size:1.2rem;">📊 Progression : {total_rep}/10</span><br>
            <span style="color:white;font-size:2rem;font-weight:bold;">Score actuel : {score_tmp}/{total_rep}</span>
        </div>""", unsafe_allow_html=True)
...
# ========= PAGE 4 ==================================================
    ...
    input_raw = [st.session_state.reponses.get(f"A{i}", 0) for i in range(1,11)] + \
                [inf.get("age",5), 0 if inf.get("genre")=="garcon" else 1, ethnie_map.get(inf.get("ethnie"),0), inf.get("jaundice",0), inf.get("family_asd",0)]
    sum_sc = int(sum(input_raw[:10]))
    age_val = inf.get("age",5)
    age_grp = (0 if age_val<=4 else 1 if age_val<=12 else 2 if age_val<=18 else 3 if age_val<=40 else 4)
    ...
    for idx, (nom, res) in enumerate(sorted(ar.items(), key=lambda x: x[1]["auc"], reverse=True)):
        ...
        try:
            if nom in ("🧠 ANN", "📡 CNN"):
                ...
                prob = float(clf.predict_proba(vec_scaled)[0][1]) if hasattr(clf,"predict_proba") else float(pred)
        except Exception:
            pred, prob = 0, 0.0
        ...
        with cols3[idx % 3]:
            st.markdown(f"""
            <div style="background:{color};border-radius:12px; padding:.8rem;margin-bottom:.8rem;text-align:center;">
    ...
    st.markdown(f"""
    <div style="background:linear-gradient(135deg,#667eea,#764ba2); border-radius:20px;padding:1.2rem;text-align:center;margin:1rem 0;">
        <h3 style="color:white;">🗳️ {votes_asd}/{len(ar)} modèles détectent un risque</h3>
    </div>""", unsafe_allow_html=True)
    st.markdown("---")

    best_pred, best_prob = model_preds.get(bn, (0,0.0))
    st.markdown(f'<div class="winner-box">🏆 Décision finale — {bn}</div>', unsafe_allow_html=True)

    if best_pred == 1:
        st.markdown("""
        <div class="result-card fade-in" style="background:linear-gradient(135deg,#f093fb,#f5576c);">
            <span style="font-size:4rem;">🚨</span>
            <h1 style="color:white;">Risque élevé détecté</h1>
            <p style="color:white;font-size:1.2rem;">Une évaluation clinique approfondie est recommandée</p>
        </div>""", unsafe_allow_html=True)
    else:
        st.markdown("""
        <div class="result-card fade-in" style="background:linear-gradient(135deg,#84fab0,#8fd3f4);">
            <span style="font-size:4rem;">✅</span>
    ...
    # Graphiques (version simplifiée mais complète)
    ...
    for bar, val in zip(bars, aucs_s):
        ax_cmp.text(val+.003, bar.get_y()+bar.get_height()/2, f"{val:.3f}", va="center", fontsize=9)
    ax_cmp.axvline(.8, color="red", ls="--", alpha=.5, label="Seuil 0.80")
    ax_cmp.set_xlim(0,1.08)
    ax_cmp.set_xlabel("ROC-AUC")
    ax_cmp.set_title("Comparaison de tous les modèles")
    ...
    with cg1:
        ...
        sns.heatmap(cm, annot=True, fmt="d", cmap="Blues", ax=ax_cm, xticklabels=["Non-ASD","ASD"], yticklabels=["Non-ASD","ASD"])
        ax_cm.set_xlabel("Prédiction"); ax_cm.set_ylabel("Réalité")
        st.pyplot(fig_cm)
    with cg2:
        st.markdown("#### 📈 Rapport de classification")
        rdf = pd.DataFrame(classification_report(st.session_state.y_test, st.session_state.y_pred, output_dict=True)).transpose().round(3)
    ...
    for (nom, res), col in zip(sorted(ar.items(), key=lambda x: x[1]["auc"], reverse=True), palette):
        if res["y_proba"] is not None:
            fpr_r, tpr_r, _ = roc_curve(st.session_state.y_test, res["y_proba"])
            lw = 3 if nom==bn else 1.5
            ls = "-" if nom==bn else "--"
            ax_roc.plot(fpr_r, tpr_r, lw=lw, ls=ls, color=col, label=f"{nom} ({res['auc']:.3f})")
    ...
    # Learning curve meilleur modèle
    ...
    if not hasattr(best_clf, "predict_proba") or bn in ("🧠 ANN","📡 CNN"):
        st.info("ℹ️ Learning Curve disponible pour modèles scikit-learn.")
    else:
        try:
    ...
    # Historique deep learning (si TF_OK)
    if TF_OK:
    ...
    if best_pred == 1:
        msg = ("• Consultez rapidement un pédiatre ou neuropédiatre<br>• Contactez un centre de référence pour l'autisme<br>• Notez les comportements observés") if role=="parent" else ("• Réalisez une évaluation clinique approfondie (ADOS, CARS, M-CHAT)<br>• Orientez vers un centre spécialisé si nécessaire")
        st.markdown(f'<div style="background:#fff3cd;border-left:4px solid #ffc107;padding:1rem;border-radius:10px;"><strong>⚠️ Recommandations :</strong><br>{msg}</div>', unsafe_allow_html=True)
    else:
        msg = ("• Continuez à surveiller le développement de votre enfant<br>• Consultez régulièrement votre pédiatre") if role=="parent" else ("• Rassurez les parents, le développement semble dans la norme<br>• Continuez le suivi régulier")
        st.markdown(f'<div style="background:#d4edda;border-left:4px solid #28a745;padding:1rem;border-radius:10px;"><strong>✅ Recommandations :</strong><br>{msg}</div>', unsafe_allow_html=True)
...
<div style="text-align:center;padding:2rem;color:rgba(255,255,255,.7);">
    ...
    <p style="font-size:.8rem;">© 2024 — Outil d'aide à la décision — Consultez toujours un professionnel de santé</p>
</div>
""", unsafe_allow_html=True)
